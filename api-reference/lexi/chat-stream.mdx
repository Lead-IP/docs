...
| Event | Purpose |
|-------|---------|
| `thinking` | Intermediate reasoning step. `data: { "text": "..." }`. Safe to ignore if not rendering a progress UI. |
| `tool_step` | Lexi is about to invoke or has completed a CRM tool. `data: { "tool": "...", "status": "started|completed", "summary": "..." }`. |
| `text` | Incremental assistant text. `data: { "delta": "...", "conversation_id": "..." }`. Concatenate `delta` values to build the final response. |
| `done` | Final event with usage metadata. `data: { "credits_remaining": 4840, "message_id": "msg_01HY1" }`. |
